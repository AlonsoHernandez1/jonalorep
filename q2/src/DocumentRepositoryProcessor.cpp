...
bool DocumentRepositoryProcessor::isDatDocument(std::string filePath) {
  return filePath.substr(filePath.find_last_of('.')+1).compare(binaryFileExtension)==0;
}
...
void DocumentRepositoryProcessor::processFile(string filePath) {
  ...
  if(isTxtDocument(filePath)) {
    ...
    //Delete WordList
    delete wordList;
  } else if (isDatDocument(filePath)){
    cout << "to do: index binary file " << filePath << endl;
  } else {
    //Not a txt file.
